...
export const store = async (req: Request, res: Response): Promise<Response> => {
  ...
  try {
    ...
    return res.status(500).json({ error: 'No se pudo almacenar el TAG.' });
  }
};
...
export const remove = async (req: Request, res: Response): Promise<Response> => {
  ...
  try {
    ...
    return res.status(200).json({ message: 'Ticket tag eliminado exitosamente' });
  } catch (error) {
    return res.status(500).json({ error: 'Falla al borrar el ticket TAG' });
  }
};