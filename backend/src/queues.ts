...
async function handleVerifyQueue(job) {
  logger.info("Buscando tickets perdidos");
  try {
    ...
    if (campaign) {
      ...
      if (isArray(contacts)) {
        ...
        for (let contact of contacts) {
          ...
          logger.info(
            `Registro enviado a la cola de activación: Campaña=${campaign.id};Contato=${contact.name};delay=${delay}`
          );
          index++;
        ...
        await campaign.update({ status: "EN_CURSO" });
      }
    }
  } catch (err: any) {
    Sentry.captureException(err);
