      ...
      if (content.tag === "terminate") {
        ...
        if (sendMsgCall.value === "disabled") {
          await wbot.sendMessage(node.attrs.from, {
            text:
              "*Mensaje Automático:*\n\nLas llamadas de voz o video estan desabilitadas para este WhatsApp, por favor enviar solo mensajes, Gracias!",
          });
          ...
          const body = `Llamada de voz o video perdida: ${hours}:${minutes}`;
          const messageData = {
