      ...
      if (isNumeric(numberCPFCNPJ) === true) {
        if (cpfcnpj.length > 2) {
          ...
          if (isCPFCNPJ) {
                ...
                axios.request(config as any)
                  .then(async function (response) {
                    if (response.data == 'NULL') {
                      const textMessage = {
                        text: formatBody(`¡Registro no encontrado! *N° o Caracteristica incorrecto o no válido*. ¡Intenta nuevamente!`, contact),
                      };
                      try {
                        await sleep(2000)
                        await wbot.sendMessage(`${ticket.contact.number}@${ticket.isGroup ? "g.us" : "s.whatsapp.net"}`, textMessage);
                      ...
                      try {
                        const textMessage = { text: formatBody(`¡Encontré tu registro! *${nome}* ¡Solo un momento más por favor!`, contact) };
                        await wbot.sendMessage(`${ticket.contact.number}@${ticket.isGroup ? "g.us" : "s.whatsapp.net"}`, textMessage);
                        const bodyBoleto = { text: formatBody(`¡Sigue el duplicado de tu Factura!\n\n*Nombre:* ${nome}\n*Valor:* $ ${valorCorrigido}\n*Fecha Vencimiento:* ${anoMesDia}\n*Link:* ${urlmkauth}/boleto/21boleto.php?titulo=${titulo}\n\n¡Te enviaré el *código de barras* en el siguiente mensaje para que te resulte más fácil copiarlo!`, contact) };
                        await sleep(2000)
                        ...
                        if (qrcode !== null) {
                          const bodyPdf = { text: formatBody(`Esto es *COPIAR Y PEGAR PIX*`, contact) };
                          await sleep(2000)
                        ...
                        if (bloqueado === 'sim') {
                          ...
                          const bodyqrcode = { text: formatBody(`Te concedo acceso. ¡Espere por favor!`, contact) };
                          await sleep(2000)
                          await wbot.sendMessage(`${ticket.contact.number}@${ticket.isGroup ? "g.us" : "s.whatsapp.net"}`, bodyqrcode);
                          var optionsdesbloq = {
                            method: 'GET',
                          ...
                          axios.request(optionsdesbloq as any).then(async function (response) {
                            const bodyLiberado = { text: formatBody(`¡Lo lanzaré pronto! Voy a necesitar que *desconectes* tu equipo.\n\n*NOTA: Simplemente desconéctelo.* \n¡Espere 1 minuto y vuelva a enchufarlo!`, contact) };
                            await sleep(2000)
                            await wbot.sendMessage(`${ticket.contact.number}@${ticket.isGroup ? "g.us" : "s.whatsapp.net"}`, bodyLiberado);
                            const bodyqrcode = { text: formatBody(`¡Comprueba si tu acceso ha vuelto! Si no has regresado, ¡vuelve a ponerte en contacto y habla con un asistente!`, contact) };
                            await sleep(2000)
                            await wbot.sendMessage(`${ticket.contact.number}@${ticket.isGroup ? "g.us" : "s.whatsapp.net"}`, bodyqrcode);
                          }).catch(async function (error) {
                            const bodyfinaliza = { text: formatBody(`¡Ups! ¡Algo malo paso! Escriba *#* para volver al menú anterior y hablar con un asistente.`, contact) };
                        ...
                        console.log('11 No fue posible enviar el mensaje')
                      }
                    }
                  })
                  .catch(async function (error) {
                    try {
                      ...
                      console.log('111 No fue posible enviar el mensaje')
                    }
            ...
            const body = { text: formatBody(`Este DNI no es valido!\n\nPor favor intente nuevamente\nEscriba *#* para volver al menú anterior y hablar con un asistente.*`, contact) };
            await sleep(2000)
        ...
        if (cpfcnpj.length > 2) {
          ...
          if (isCPFCNPJ) {
            ...
            axios.request(optionsc as any).then(async function (response) {
              ...
              if (totalCount === 0) {
                const body = {
                  text: formatBody(`¡Registro no encontrado! *DNI incorrecto o no válido*. ¡Intenta nuevamente!`, contact),
                };
                await sleep(2000)
                await wbot.sendMessage(`${ticket.contact.number}@${ticket.isGroup ? "g.us" : "s.whatsapp.net"}`, body);
              } else {
                ...
                axios.request(optionsListpaymentOVERDUE as any).then(async function (response) {
                  ...
                  if (totalCount_overdue === 0) {
                    const body = {
                      text: formatBody(`¡No tienes facturas vencidas! \nTe enviaré la próxima factura. ¡Espere por favor!`, contact),
                    };
                    await sleep(2000)
                    await wbot.sendMessage(`${ticket.contact.number}@${ticket.isGroup ? "g.us" : "s.whatsapp.net"}`, body);
                    var optionsPENDING = {
